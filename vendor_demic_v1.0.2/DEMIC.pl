	...
	sub cov3_estPTR_parallel {
		...
		for my $cov3s (@cov3s_th) {
			...
			if ($test_line_num == $valid_line_num) {
				...
				my $output_R = $out_dir;
				my $log_level = "DEBUG";
				my $command = "Rscript $estPTR_path $input_R $output_R $max_candidate_iter $log_level 2>>$log";
				...
				if (@line > 10) {
					...
					if (defined $pre_contig and $pre_contig ne $line[2]) {
						if (exists $contig_len{$pre_contig}) {
							...
							my (@cov_step);
							my $n = 0;
							my $first_i = ${$GC_windows{$pre_contig}}[0][0]; # The start position of the first window for this contig
							my $last_i = ${$GC_windows{$pre_contig}}[-1][1]; # End position of the last window for this contig
							my $first_step = int(($first_i-1-$edge_length)/$window_step);
							my $last_step = int(($last_i-1-$edge_length)/$window_step); # Sort of normalized by window_step, end up with a range 0 - n where n is the number of windows

							my @info_qualified;
							for my $step ( $first_step .. $last_step ) { # Iterating over each window for the contig
								...
								if ( @cov_step == int($window_size/$window_step) ) {
									$n ++; # Iterating over each window for the contig
									...
									#output
									my $GC_window_sum = ${$info}[2]; # G+C Content
									my $GC_window_diff = ${$info}[3]; # G-C Skew
