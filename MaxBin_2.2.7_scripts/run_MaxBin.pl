...
require("$currdir\/_getmarker.pl");
require("$currdir\/_getabund.pl");
require("$currdir\/_sepReads.pl");
...
my $RSCRIPT = "Rscript";
my $HEATMAP_R = "$currdir\/heatmap.r";
my $MARKERHMM = "$currdir/marker.hmm";
my $MARKERNUM = 107;
my $MAXBIN = "MaxBin";
my $ABUND_OUTPUT = "";
my $THREADNUM = 1;
	...
	for ($i = 0; $i < $ARGC; $i++)
	{
		...
		elsif ($ARGV[$i] eq "-markerset")
		{
			...
			if ($ARGV[$i] == 40)
			{
				...
				$MARKERHMM = "$currdir/bacar_marker.hmm";
				$MARKERNUM = 40;
			}
		}
